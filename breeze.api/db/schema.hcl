...
table "assets" {
  ...
  column "home_growth_profile" {
    type = varchar(32)
    null = true
  }

  column "vehicle_depreciation_profile" {
    type = varchar(32)
    null = true
  }

  column "linked_liability_id" {
    type = uuid
    null = true
  }
...
table "planner_people" {
  ...
  index "idx_planner_people_user_id" {
    columns = [column.user_id]
  }

  index "idx_planner_people_user_active" {
    columns = [column.user_id, column.created_at]
    where   = "deleted_at IS NULL"
  }
}
