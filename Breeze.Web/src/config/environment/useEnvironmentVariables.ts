...
export const useEnvironmentVariables = (): EnvironmentVariables => {
    ...
    const baseHostedApi = getValue('BASE_HOSTED_API', 'https://breeze-apiapp.azurewebsites.net')
    const baseLocalUrl = getValue('BASE_LOCAL_URL', 'http://localhost:5173/')
    const baseHostedUrl = getValue('BASE_HOSTED_URL', 'https://breezebudgeting.azurewebsites.net/')
    const baseLocalLoginUrl = getValue('BASE_LOCAL_LOGIN_URL', 'http://localhost:5173/login')
    const baseHostedLoginUrl = getValue('BASE_HOSTED_LOGIN_URL', 'https://breezebudgeting.azurewebsites.net/login')
