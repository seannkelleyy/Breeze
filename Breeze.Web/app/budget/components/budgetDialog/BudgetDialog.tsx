...
import { useCategories, useDeleteCategory, usePatchCategory, usePostCategory } from '../../hooks/category/index'
import { useDeleteIncome, useIncomes, usePatchIncome, usePostIncome } from '../../hooks/income/index'
...
export const BudgetDialog = () => {
	...
	const onSubmit = (values: BudgetFormData) => {
		...
		values.incomes.forEach((income) => {
			...
			const normalizedMonthlyPayday =
				typeof income.paydayDayOfMonth === 'number' && income.paydayDayOfMonth >= 1 && income.paydayDayOfMonth <= 31 ? income.paydayDayOfMonth : fallbackPayday
	...
	const templateIncomeTotal = (incomes ?? []).filter((income) => income.sourceType === 'recurring-template').reduce((sum, income) => sum + (Number(income.amount) || 0), 0)
	...
	const inputFields = (
		<>
			{(incomes ?? []).some((income) => income.sourceType === 'recurring-template') || (categories ?? []).some((category) => category.sourceType === 'recurring-template') ? (
			...
			<section className='grid grid-cols-1 sm:grid-cols-3 gap-3 py-1'>
				...
				<div className='rounded-lg border p-4 bg-background/70'>
					...
					<p className={`text-2xl font-semibold mt-1 ${isTotalBudgetDifferencePositive ? 'text-green-600' : 'text-destructive'}`}>${totalBudgetDifference.toFixed(2)}</p>
				</div>
			</section>
