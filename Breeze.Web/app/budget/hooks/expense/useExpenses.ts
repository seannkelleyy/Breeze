...
const useExpenses = () => {
  ...
  const getExpensesForBudget = async (budgetId: number): Promise<Expense[]> =>
    await getMany<Expense>(`budgets/${budgetId}/expenses`);
  ...
  const patchExpense = async (budgetId: number, expense: Expense): Promise<number> =>
    patch<number, Expense>(
      `budgets/${budgetId}/categories/${expense.categoryId}/expenses`,
      expense,
    );

  const deleteExpense = async (budgetId: number, expense: Expense) =>
    deleteOne<Expense>(
      `budgets/${budgetId}/categories/${expense.categoryId}/expenses/${expense.id}`,
    );
