...
const usePlanner = () => {
  ...
  const getLatestBudgetMonthlyExpenses = async (): Promise<number> =>
    await getOne<number>('planner/latest-budget-expenses');

  const upsertPlanner = async (payload: PlannerUpsertRequest): Promise<number> =>
    put<number, PlannerUpsertRequest>('planner', payload);
