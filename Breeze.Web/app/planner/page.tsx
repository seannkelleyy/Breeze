...
import { PLANNER_DEFAULT_COLLAPSED_SECTIONS, PLANNER_DEFAULT_INCOME_REPLACEMENT_RATE } from './lib/constants'
...
const Planner = () => {
	...
	const getDefaultAssetFinanceDetails = useCallback((account: PlannerAccount): AssetFinanceDetails => getDefaultAssetFinanceDetailsForAccount(account), [])
	...
	const createPlannerPayload = useCallback(
		(
			...
			accounts: nextAccounts.map((account) => {
				const details = isCombinedAssetType(account.accountType) ? (nextAssetFinanceDetailsByAccountId[account.id] ?? getDefaultAssetFinanceDetails(account)) : undefined
	...
	const [collapsedSections, setCollapsedSections] = useState<Record<SectionKey, boolean>>({ ...PLANNER_DEFAULT_COLLAPSED_SECTIONS })
	...
	return (
		<div className='min-h-screen w-full pb-12 max-w-6xl mx-auto px-4 pt-24 flex flex-col gap-6'>
			<div>
				<h1 className='text-3xl font-bold'>Net Worth + Retirement Planner</h1>
				<p className='text-muted-foreground mt-1'>Track all account types in one place and compare your retirement target against your projected net worth.</p>
				...
				{hasPlannerSaveError ? <p className='text-sm text-destructive mt-2'>Changes failed to save. Please verify the API is running and try again.</p> : null}
			</div>
