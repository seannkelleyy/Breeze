...
import { PLANNER_ACCOUNT_RATE_PROFILE_OPTIONS, PLANNER_HOME_GROWTH_PROFILE_OPTIONS, PLANNER_VEHICLE_DEPRECIATION_PROFILE_OPTIONS } from './constants'
...
export const homeGrowthProfileOptions: ReadonlyArray<{ value: HomeGrowthProfile; label: string }> = PLANNER_HOME_GROWTH_PROFILE_OPTIONS.map((option) => ({ ...option }))

export const vehicleDepreciationProfileOptions: ReadonlyArray<{ value: VehicleDepreciationProfile; label: string }> = PLANNER_VEHICLE_DEPRECIATION_PROFILE_OPTIONS.map(
	(option) => ({ ...option }),
)

export const accountRateProfileOptions: ReadonlyArray<{ value: AccountRateProfile; label: string }> = PLANNER_ACCOUNT_RATE_PROFILE_OPTIONS.map((option) => ({ ...option }))
