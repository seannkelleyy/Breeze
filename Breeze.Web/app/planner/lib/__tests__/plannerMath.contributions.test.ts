...
describe('getEmployeeMonthlyContribution', () => {
  const selfIncome = 120000;
  const spouseIncome = 80000;

  it('returns 0 for non-contributing account types', () => {
    const account = makeAccount({ accountType: 'home' });
    expect(getEmployeeMonthlyContribution(account, selfIncome, 0)).toBe(0);
  });
  ...
  it('calculates monthly contribution mode', () => {
    ...
    expect(getEmployeeMonthlyContribution(account, selfIncome, 0)).toBe(500);
  });
  ...
  it('calculates yearly contribution mode', () => {
    ...
    expect(getEmployeeMonthlyContribution(account, selfIncome, 0)).toBe(1000);
  });
  ...
  it('uses spouse income when owner is spouse', () => {
    const account = makeAccount({
      contributionMode: 'salary-percent',
    ...
    const result = getEmployeeMonthlyContribution(account, selfIncome, spouseIncome);
...
describe('getEmployerMatchMonthly', () => {
  it('returns 0 when match rate is 0', () => {
    ...
    expect(getEmployerMatchMonthly(account, 120000, 0)).toBe(0);
  });
  ...
  it('calculates 50% match up to 6% of salary', () => {
    ...
    // Monthly: 3600 / 12 = 300
    expect(getEmployerMatchMonthly(account, 120000, 0)).toBeCloseTo(300, 0);
  });
