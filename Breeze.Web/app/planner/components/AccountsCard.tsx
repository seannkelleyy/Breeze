...
const AccountsCard = ({ collapsed, toggleControl }: AccountsCardProps) => {
	...
	const filteredAccounts = useMemo(() => {
		switch (accountFilter) {
			case 'assets':
				return plannerAccounts.filter((account: PlannerAccount) => !isLiabilityAccountType(account.accountType))
			case 'liabilities':
			...
			case 'tax-advantaged':
				return plannerAccounts.filter((account: PlannerAccount) => getSuggestedAnnualLimitForAccount(account.accountType, 40, hasSpouse) > 0)
			case 'all':
			default:
				return plannerAccounts
		}
	}, [accountFilter, plannerAccounts, assetFinanceDetailsByAccountId, getSuggestedAnnualLimitForAccount, hasSpouse, isCombinedAssetType, isLiabilityAccountType])
		...
		<Card>
			<CardHeader className='flex flex-row items-start justify-between gap-2'>
				<div>
					...
					<CardDescription>Add investment and non-investment assets (home, vehicle, emergency fund, checking), contributions, and growth assumptions.</CardDescription>
				</div>
				{toggleControl}
			</CardHeader>
			{!collapsed ? (
				<CardContent className='flex flex-col gap-4'>
					...
					<div className='grid grid-cols-1 xl:grid-cols-2 gap-4'>
						{filteredAccounts.map((account: PlannerAccount) => {
							...
							const ownerAge = getAgeFromBirthday((people.find((person: PlannerPerson) => person.type === account.owner)?.birthday ?? selfBirthday) || '')
							...
							const onUpdateAssetFinanceDetails = (updater: (current: AssetFinanceDetails) => AssetFinanceDetails) => updateAssetFinanceDetails(account.id, updater)
							...
							return (
								<div
									key={account.id}
									className='border rounded-md p-3 space-y-3 h-fit'
								>
									...
									>
										<div className='space-y-2'>
											...
											<Input
												value={account.name}
												onChange={(event) => onUpdateAccount((current) => ({ ...current, name: event.target.value }))}
											/>
										</div>
										<div className='space-y-2'>
											<Label>Owner</Label>
									...
									<div className='text-xs text-muted-foreground'>
										{isAccountCollapsed ? (
											<p>
												...
												{assetFinanceSnapshot ? `, Equity: ${formatCurrency(assetFinanceSnapshot.equity)}` : `, Balance: ${formatCurrency(account.startingBalance)}`}
											</p>
										) : null}
										{hidesContributionInputs ? (
											...
											<>
												...
												{suggestedLimit > 0 ? (
													<span className={plannerConstants.isMoneyGreaterThanWithTolerance(employeeAnnual, suggestedLimit) ? 'text-destructive font-medium' : ''}>
										...
										{usesDepreciationInput ? (
											<span className='ml-2'>Vehicle depreciation uses a tapered curve by age (faster early years, slower later years), unless Custom is selected.</span>
										) : null}
										{account.accountType === '401k' ? <span className='ml-2'>Employer match applied monthly: {formatCurrency(employerMatchMonthly)}</span> : null}
									</div>
								</div>
							)
						})}
						{filteredAccounts.length === 0 ? <div className='xl:col-span-2 border rounded-md p-4 text-sm text-muted-foreground'>No accounts match this filter.</div> : null}
					</div>
					...
					<p className='text-xs text-muted-foreground'>401(k) formula: Employer match = min(employee annual contribution, salary x match cap %) x match rate %.</p>
				</CardContent>
			) : null}
		</Card>
	)
