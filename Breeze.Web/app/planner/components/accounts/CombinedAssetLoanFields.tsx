...
const CombinedAssetLoanFields = ({ assetFinanceDetails, onUpdateAssetFinanceDetails, formatCurrency }: CombinedAssetLoanFieldsProps) => {
	return (
		<>
				...
				<Select
					...
					onValueChange={(value) =>
						onUpdateAssetFinanceDetails((current) => ({
							...current,
							hasLoan: value === 'yes',
							originalLoanAmount: value === 'yes' && clamp(current.originalLoanAmount) <= 0 ? clamp(current.currentLoanBalance) : current.originalLoanAmount,
						}))
					}
				>
					<SelectTrigger>
				...
				<>
					...
					{assetFinanceDetails.originalLoanAmount > 0 ? (
						<div className='md:col-span-2 rounded-md border p-2 text-xs text-muted-foreground'>
							<p>Principal paid to date: {formatCurrency(Math.max(0, assetFinanceDetails.originalLoanAmount - assetFinanceDetails.currentLoanBalance))}</p>
							<p>
								Loan paid off:{' '}
								{((Math.max(0, assetFinanceDetails.originalLoanAmount - assetFinanceDetails.currentLoanBalance) / assetFinanceDetails.originalLoanAmount) * 100).toFixed(1)}%
							</p>
						</div>
					) : null}
				</>
