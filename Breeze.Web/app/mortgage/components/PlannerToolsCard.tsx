...
export const PlannerToolsCard = ({ collapsed, toggleControl, formatCurrency, homeLoan }: PlannerToolsCardProps) => {
	...
	const refinanceSummary = useMemo(() => {
		...
		const recommendationLabel = recommendationScore >= 45 ? 'Recommend Refinance' : recommendationScore >= 10 ? 'Borderline / Depends on Priorities' : 'Keep Current Loan'
	...
	const extraPaymentSummary = useMemo(() => {
		...
		const acceleratedSummary = buildAmortization(homeLoan.currentBalance, homeLoan.interestRate, homeLoan.monthlyPayment, new Date(), extraOneTime, extraMonthly)
