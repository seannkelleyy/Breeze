      ...
      {homeLoan ? (
        <>
          <p className="text-muted-foreground text-sm">
            {homeLoan.accountName} balance {formatCurrency(homeLoan.currentBalance)} at{' '}
            {homeLoan.interestRate.toFixed(2)}%
          </p>
          {homeLoan.originalLoanAmount && homeLoan.originalLoanAmount > 0 ? (
            <p className="text-muted-foreground text-sm">
              Original loan: {formatCurrency(homeLoan.originalLoanAmount)}
              {loanPaidDownPercent !== null
                ? `, Paid down: ${loanPaidDownPercent.toFixed(1)}%`
                : ''}
            </p>
          ) : null}
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {compareTerms.map((scenario) => (
              <div key={scenario.termYears} className="space-y-1 rounded-md border p-3 text-sm">
                ...
                <p>
                  Payoff:{' '}
                  {scenario.payoffDate ? monthDateFormatter.format(scenario.payoffDate) : 'N/A'}
                </p>
