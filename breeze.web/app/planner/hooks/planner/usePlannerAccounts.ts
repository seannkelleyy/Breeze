...
const usePlannerAccounts = () => {
  ...
  const getSuggestedAnnualLimitForAccount = (accountType: AccountType, age: number) =>
    getSuggestedAnnualLimit(accountType, age, irsLimits, people.length > 1);
  ...
  const updateAssetFinanceDetails = (
    ...
    setPlannerAssetFinanceDetailsByAccountId((prev) => {
      ...
      updateAccount(accountId, (c) => ({
        ...
        purchaseDate: nextDetails.purchaseDate ?? null,
        purchasePrice: nextDetails.purchasePrice,
        homeGrowthProfile: nextDetails.homeGrowthProfile ?? null,
        vehicleDepreciationProfile: nextDetails.vehicleDepreciationProfile ?? null,
      }));
